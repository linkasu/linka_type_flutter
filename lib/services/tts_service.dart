...
class TTSService {
  ...
  Future<void> _downloadAudio(Uint8List bytes, String text) async {
    try {
      ...
      final fileName =
          'LINKa. напиши. ${text.substring(0, text.length > 5 ? 5 : text.length)}.mp3';
  ...
  Future<void> setVoice(String voiceURI) async {
    ...
    try {
      ...
      final yandexVoice = yandexVoices.firstWhere(
      ...
      try {
        ...
        final offlineVoice = offlineVoices.firstWhere(
          (v) => v.voiceURI == voiceURI,
        );
        ...
        await _flutterTts?.setVoice({
          "name": voiceURI,
          "locale": offlineVoice.locale ?? "ru-RU",
        });
      } catch (e2) {
        ...
        if (yandexVoices.isNotEmpty) {
          ...
          await _flutterTts?.setVoice({
            "name": selectedVoice.voiceURI,
            "locale": selectedVoice.locale ?? "ru-RU",
          });
        }
      }
    }
  ...
  Future<TTSVoice> getSelectedVoice() async {
    ...
    try {
      // Сначала ищем в Яндекс голосах
      final yandexVoice = yandexVoices.firstWhere(
        (v) => v.voiceURI == savedURI,
      );
      return TTSVoice(voiceURI: yandexVoice.voiceURI, text: yandexVoice.text);
    } catch (e) {
      try {
        // Затем ищем в офлайн голосах
        final offlineVoice = offlineVoices.firstWhere(
        ...
        if (yandexVoices.isNotEmpty) {
          ...
          // Fallback
          return TTSVoice(voiceURI: 'default', text: 'По умолчанию');
        }
      }
    }
  }
    ...
    try {
      ...
      if (voices is List) {
        final voiceList = voices.map((voice) {
          if (voice is Map) {
            final Map<Object?, Object?> voiceMap =
                voice as Map<Object?, Object?>;
            return TTSVoice(
        ...
        voiceList.sort((a, b) {
          ...
          if (a.locale?.startsWith('ru') == true &&
              b.locale?.startsWith('ru') != true) return -1;
          if (b.locale?.startsWith('ru') == true &&
              a.locale?.startsWith('ru') != true) return 1;

          // Английский язык имеет второй приоритет
          if (a.locale?.startsWith('en') == true &&
              b.locale?.startsWith('en') != true) return -1;
          if (b.locale?.startsWith('en') == true &&
              a.locale?.startsWith('en') != true) return 1;
  ...
  Future<List<TTSVoice>> getRussianVoices() async {
    try {
      ...
      return allVoices
          .where((voice) => voice.locale?.startsWith('ru') == true)
          .toList();
    } catch (e) {
      return [];
    }
